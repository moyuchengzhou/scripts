-- --------------------------------------------------------------------------
-- --[[ sjy_lingdiespawner class definition ]]
-- --------------------------------------------------------------------------

-- return Class(function(self, inst)

--     assert(TheWorld.ismastersim, "sjy_lingdiespawner should not exist on client")
...
--     local _sjy_lingdies = {}
--     local _maxsjy_lingdies = 10
    
--     --------------------------------------------------------------------------
--     --[[ Private member functions ]]
--     --------------------------------------------------------------------------
    
--     local FLOWER_TAGS = { "gravestone" }
--     -- local BUTTERFLY_TAGS = { "sjy_lingdie" }
--     local SJY_LINGDIE_TAGS = { "sjy_lingdie" }
    
--     local function SJYGetSpawnPoint(player)
--         local rad = 25
...
--     local function sjy_lingdieSpawnBu(player, reschedule)
--         local pt = player:GetPosition()
--         local ents = TheSim:FindEntities(pt.x, pt.y, pt.z, 64, SJY_LINGDIE_TAGS)
--         if #ents < _maxsjy_lingdies then
--             local spawnflower = SJYGetSpawnPoint(player)
--             if spawnflower ~= nil then
--                 local sjy_lingdie = SpawnPrefab("sjy_lingdie")
--                 if sjy_lingdie.components.pollinator ~= nil then
--                     sjy_lingdie.components.pollinator:Pollinate(spawnflower)
--                 end
--                 sjy_lingdie.components.homeseeker:SetHome(spawnflower)
--                 -- KAJ: TODO: Butterflies can be despawned before getting to the rest of the logic if this is above the homeseeker
--                 sjy_lingdie.Physics:Teleport(spawnflower.Transform:GetWorldPosition())
--             end
--         end
...
--             _scheduledtasks[player] = player:DoTaskInTime(basedelay + math.random() * 10, sjy_lingdieSpawnBu, ScheduleSpawn)
--         end
--     end
...
--         if _worldstate.isday and not _worldstate.iswinter and _maxsjy_lingdies > 0 then
...
--         if _sjy_lingdies[target] ~= nil and target:IsAsleep() then
--             target:Remove()
--         end
--     end
...
--         if _sjy_lingdies[target] == nil then
...
--         local numsjy_lingdies = 0
--         for k, v in pairs(_sjy_lingdies) do
--             numsjy_lingdies = numsjy_lingdies + 1
--         end
--         return string.format("updating:%s sjy_lingdies:%d/%d", tostring(_updating), numsjy_lingdies, _maxsjy_lingdies)
--     end
