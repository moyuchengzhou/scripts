...
-- 加载贴图，动画
STRINGS.CHARACTERS.GENERIC.DESCRIBE.CZSCDR = "热乎乎的炖菜!!!"

local assets = {Asset("ANIM", "anim/czscdr.zip"), Asset("IMAGE", "images/czscdr.tex"), Asset("ATLAS", "images/czscdr.xml")}
...
local function oneaten(inst, eater)
    if eater.components.timer and eater:HasTag("player") then -- 判断是玩家
        ...
        eater.components.talker:Say("将冷鲜肉和蔬菜切作小块，加上番茄久煮炖成浓稠的菜肴!!!!")		
            end
        end
end

function fn()
    local assetname = "czscdr"
    ...
    inst.components.inventoryitem.atlasname = "images/czscdr.xml" -- 在背包里的贴图
...
return Prefab("czscdr", fn, assets, prefabs)