local assets =
{
    Asset("ANIM", "anim/banlibush.zip"),
    -- Asset("ANIM", "anim/rock_avocado_build.zip"),
    -- Asset("PKGREF", "anim/rock_avocado_diseased_build.zip"),
    Asset("IMAGE", "images/inventoryimages/banliico.tex"),
	Asset("ATLAS", "images/inventoryimages/banliico.xml"),
}
...
local function set_stage3(inst)
    inst.components.pickable:ChangeProduct("banlibean")
...
local function on_bush_burnt(inst)
    ...
    if inst.components.growable.stage == 3 then
        inst.components.lootdropper:SpawnLootPrefab("banlibean")
        inst.components.lootdropper:SpawnLootPrefab("banlibean")
        inst.components.lootdropper:SpawnLootPrefab("banlibean")
    end
...
local function on_dug_up(inst, digger)
    ...
    if withered or inst.components.pickable:IsBarren() then
        ...
        inst.components.lootdropper:SpawnLootPrefab("dug_banlibush")
    end
...
local function banlibush()
    ...
    inst.AnimState:SetBank("banlibush")
    inst.AnimState:SetBuild("banlibush")
    ...
    inst.MiniMapEntity:SetIcon("banliico.tex")
...
return Prefab("banlibush", banlibush, assets, prefabs)
