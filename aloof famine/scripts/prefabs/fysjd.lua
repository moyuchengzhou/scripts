...
-- 加载贴图，动画
STRINGS.CHARACTERS.GENERIC.DESCRIBE.FYSJD = "清蒸料理"

local assets = {Asset("ANIM", "anim/fysjd.zip"), Asset("IMAGE", "images/fysjd.tex"), Asset("ATLAS", "images/fysjd.xml")}
...
local function oneaten(inst, eater)
    if eater.components.timer and eater:HasTag("player") then -- 判断是玩家
        ...
        eater.components.talker:Say("大肉和土豆打成泥和成面团，将面团擀成一张张薄皮，在把肉泥包裹住，再用海带打个结，淋上翠绿的石果汁清蒸!!!")		
            end
        end
end

function fn()
    local assetname = "fysjd"
    ...
    inst.components.inventoryitem.atlasname = "images/fysjd.xml" -- 在背包里的贴图
...
return Prefab("fysjd", fn, assets, prefabs)