...
-- 加载贴图，动画
STRINGS.CHARACTERS.GENERIC.DESCRIBE.XXYBM = "海鲜蔬菜料理"

local assets = {Asset("ANIM", "anim/xxybm.zip"), Asset("IMAGE", "images/xxybm.tex"), Asset("ATLAS", "images/xxybm.xml")}
...
local function oneaten(inst, eater)
    if eater.components.timer and eater:HasTag("player") then -- 判断是玩家
        ...
        eater.components.talker:Say("Q弹的水晶虾仁，拌上蔬菜熬制的鱼肉高汤，将洋葱切片放入去除鱼腥味让口感更加清甜!!!")		
            end
        end
end

function fn()
    local assetname = "xxybm"
    ...
    inst.components.inventoryitem.atlasname = "images/xxybm.xml" -- 在背包里的贴图
...
return Prefab("xxybm", fn, assets, prefabs)