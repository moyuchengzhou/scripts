...
-- 加载贴图，动画
STRINGS.CHARACTERS.GENERIC.DESCRIBE.BWWNJ = "蜜酿后烤熟的禽肉"

local assets = {Asset("ANIM", "anim/bwwnj.zip"), Asset("IMAGE", "images/bwwnj.tex"), Asset("ATLAS", "images/bwwnj.xml")}
...
local function oneaten(inst, eater)
    if eater.components.timer and eater:HasTag("player") then -- 判断是玩家
        ...
        --     eater.components.talker:Say("肉质如同流蜜一般香甜软糯。叫人想把每一根骨头都吮吸干净!!!")	
        -- end
            
    end
end

local fn = function (inst,player)
    if player.components.freezable and not player.components.freezable:IsFrozen () then
       player.components.freezable:Freeze(60) -- body
    end
end

function fn()
    local assetname = "bwwnj"
    ...
    inst.components.inventoryitem.atlasname = "images/bwwnj.xml" -- 在背包里的贴图
...
return Prefab("bwwnj", fn, assets, prefabs)