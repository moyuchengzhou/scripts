...
local assets = 
{
   Asset("ANIM", "anim/sjy_wsj_youlings.zip"),    
   Asset("IMAGE", "images/inventoryimages/sjy_wsj_youlings.tex"),
   Asset("ATLAS", "images/inventoryimages/sjy_wsj_youlings.xml"),--默认:雪人
}
...
function fn()
   ...
   inst.Light:SetRadius(1*2)                       --发光范围:半径2格地皮
   inst.Light:SetFalloff(1)                        --衰减
   inst.Light:SetIntensity(0.5)                   --强度
   inst.Light:SetColour(116/255, 237/255, 245/255)  --黄色

   inst.entity:AddAnimState()                      --添加动画组件
   inst.AnimState:SetBank("sjy_wsj_youlings")        --
   inst.AnimState:SetBuild("sjy_wsj_youlings")       --smcl文件的名字
   inst.AnimState:PlayAnimation("idle")            --动画子名称,播放的就是它

   inst:AddTag("sjy_wsj_youlings")                   --独有标签
...
return Prefab("sjy_wsj_youlings", fn, assets),
MakePlacer("sjy_wsj_youlings_placer", "sjy_wsj_youlings", "sjy_wsj_youlings", "idle")