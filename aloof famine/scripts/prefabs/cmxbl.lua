...
-- 加载贴图，动画
STRINGS.CHARACTERS.GENERIC.DESCRIBE.cmxbl = "清心果茶"

local assets = {Asset("ANIM", "anim/cmxbl.zip"), Asset("IMAGE", "images/cmxbl.tex"), Asset("ATLAS", "images/cmxbl.xml")}
...
local function oneaten(inst, eater) 
    if eater.wormlight ~= nil then 
        if eater.wormlight.prefab == "wormlight_light_greater" then
            ...
            eater.components.talker:Say("Q弹细腻的果肉，忍不住的散发着光芒~")
            -- eater.greaterlightfn = { WORMLIGHT_DURATION = TUNING.SEG_TIME*320 }
            return
        else
...
--         eater.components.talker:Say("细腻的奶油，香蕉的丝滑~")		
--             end
--         end
-- end

function fn()
    local assetname = "cmxbl"
    ...
    inst.components.inventoryitem.atlasname = "images/cmxbl.xml" -- 在背包里的贴图
...
return Prefab("cmxbl", fn, assets, prefabs)