...
-- 加载贴图，动画
STRINGS.CHARACTERS.GENERIC.DESCRIBE.SRBHJ = "酸辣可口的菜肴"

local assets = {Asset("ANIM", "anim/srbhj.zip"), Asset("IMAGE", "images/srbhj.tex"), Asset("ATLAS", "images/srbhj.xml")}
...
local function oneaten(inst, eater)
    if eater.components.timer and eater:HasTag("player") then -- 判断是玩家
        ...
        eater.components.talker:Say("将卤制好的兽物肉切成薄片，裹上鸡蛋卷入海带叶，并浇上麻辣酥香的蘸水")		
            end
        end
end

function fn()
    local assetname = "srbhj"
    ...
    inst.components.inventoryitem.atlasname = "images/srbhj.xml" -- 在背包里的贴图
    ...
    inst.components.edible.healthvalue = 35
...
return Prefab("srbhj", fn, assets, prefabs)