...
-- 加载贴图，动画
STRINGS.CHARACTERS.GENERIC.DESCRIBE.GGXL = "色彩素净的饮品"

local assets = {Asset("ANIM", "anim/ggxl.zip"), Asset("IMAGE", "images/ggxl.tex"), Asset("ATLAS", "images/ggxl.xml")}
...
local function oneaten(inst, eater)
    if eater.components.timer and eater:HasTag("player") then -- 判断是玩家
        ...
        eater.components.talker:Say("清新的冰沙，隐隐约约带着几丝缥缈的果味!!!")		
            end
        end
end

function fn()
    local assetname = "ggxl"
    ...
    inst.components.inventoryitem.atlasname = "images/ggxl.xml" -- 在背包里的贴图
...
return Prefab("ggxl", fn, assets, prefabs)