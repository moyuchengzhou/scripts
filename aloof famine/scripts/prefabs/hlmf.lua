...
-- 加载贴图，动画
STRINGS.CHARACTERS.GENERIC.DESCRIBE.HLMF = "外形可爱的的主食"

local assets = {Asset("ANIM", "anim/hlmf.zip"), Asset("IMAGE", "images/hlmf.tex"), Asset("ATLAS", "images/hlmf.xml")}
...
local function oneaten(inst, eater)
    if eater.components.timer and eater:HasTag("player") then -- 判断是玩家
        ...
        eater.components.talker:Say("简单的处理方法最大化保留住了食材的鲜美。就算是再不爱搭理人的小猫，闻到这样的鲜香也会忍不住跟上来吧!!!")		
            end
        end
end

function fn()
    local assetname = "hlmf"
    ...
    inst.components.inventoryitem.atlasname = "images/hlmf.xml" -- 在背包里的贴图
...
return Prefab("hlmf", fn, assets, prefabs)