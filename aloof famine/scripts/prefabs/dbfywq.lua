...
-- 加载贴图，动画
STRINGS.CHARACTERS.GENERIC.DESCRIBE.DBFYWQ = "蛋皮包裹的主食"

local assets = {Asset("ANIM", "anim/dbfywq.zip"), Asset("IMAGE", "images/dbfywq.tex"), Asset("ATLAS", "images/dbfywq.xml")}
...
local function oneaten(inst, eater)
    if eater.components.timer and eater:HasTag("player") then -- 判断是玩家
        ...
        eater.components.talker:Say("柔软的蛋皮，仿佛在酱汁构筑成的舞台上翩翩起舞，摇曳的裙摆下包裹着松软香甜的米饭!!!")		
            end
        end
end

function fn()
    local assetname = "dbfywq"
    ...
    inst.components.inventoryitem.atlasname = "images/dbfywq.xml" -- 在背包里的贴图
...
return Prefab("dbfywq", fn, assets, prefabs)