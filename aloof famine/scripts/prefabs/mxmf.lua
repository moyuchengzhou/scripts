...
-- 加载贴图，动画
STRINGS.CHARACTERS.GENERIC.DESCRIBE.mxmf = "热乎乎的炖菜!!!"

local assets = {Asset("ANIM", "anim/mxmf.zip"), Asset("IMAGE", "images/mxmf.tex"), Asset("ATLAS", "images/mxmf.xml")}
...
local function oneaten(inst, eater)
    if eater.components.timer and eater:HasTag("player") then -- 判断是玩家
        if eater.components.combat ~= nil then --这个buff需要攻击组件
            ...
            -- eater.components.talker:Say("丝滑的奶油泡芙!!!!")		
            end
...
function fn()
    ...
    inst.components.inventoryitem.atlasname = "images/mxmf.xml" -- 在背包里的贴图
...
return Prefab("mxmf", fn, assets, prefabs)