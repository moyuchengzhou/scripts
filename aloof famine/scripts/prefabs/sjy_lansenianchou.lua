...
local assets = {Asset("ANIM", "anim/sjy_lansenianchou.zip"), Asset("IMAGE", "images/sjy_lansenianchou.tex"), Asset("ATLAS", "images/sjy_lansenianchou.xml")}
...
local function oneaten(inst, eater)
    if eater.components.timer and eater:HasTag("player") then -- 判断是玩家
        if eater.components.combat ~= nil then --这个buff需要攻击组件
            ...
            eater.components.talker:Say("好恶心不能吃！")		
            end
...
function fn()
    local assetname = "sjy_lansenianchou"
    ...
    inst.components.inventoryitem.atlasname = "images/sjy_lansenianchou.xml" -- 在背包里的贴图

    inst:AddComponent("edible") -- 可食物组件
    inst.components.edible.foodtype = FOODTYPE.GOODIES

    inst:AddComponent("perishable") -- 可腐烂的组件
    inst.components.perishable:SetPerishTime(999999)
    inst.components.perishable:StartPerishing()
    inst.components.perishable.onperishreplacement = "sjy_lansenianchou" -- 腐烂后变成腐烂食物
...
return Prefab("sjy_lansenianchou", fn, assets, prefabs)