...
local assets = 
{
   Asset("ANIM", "anim/sjy_wsj_lazhunangua.zip"),
   Asset("IMAGE", "images/inventoryimages/sjy_wsj_lazhunangua.tex"),
   Asset("ATLAS", "images/inventoryimages/sjy_wsj_lazhunangua.xml"),
   --默认:雪人
}
...
function fn()
   ...
   inst.Light:SetRadius(3*2)                       --发光范围:半径2格地皮
   inst.Light:SetFalloff(1)                        --衰减
   inst.Light:SetIntensity(0.2)                   --强度
   inst.Light:SetColour(244/255, 244/255, 132/255)  --黄色

   inst.entity:AddAnimState()                      --添加动画组件
   inst.AnimState:SetBank("sjy_wsj_lazhunangua")        --
   inst.AnimState:SetBuild("sjy_wsj_lazhunangua")       --smcl文件的名字
   inst.AnimState:PlayAnimation("idle")            --动画子名称,播放的就是它

   inst:AddTag("sjy_wsj_lazhunangua")                   --独有标签
...
return Prefab("sjy_wsj_lazhunangua", fn, assets),
MakePlacer("sjy_wsj_lazhunangua_placer", "sjy_wsj_lazhunangua", "sjy_wsj_lazhunangua", "idle")